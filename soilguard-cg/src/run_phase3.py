...
from src.ml_risk import prepare_feature_matrix, train_soil_risk_model, predict_full_risk_map, MODEL_SAVE_PATH

console = Console()
OUTPUT_DIR = os.path.join(PROJECT_ROOT, "outputs", "phase3")
...
def plot_risk_score_map(risk_map, output_dir=OUTPUT_DIR):
    ...
    cbar.set_label('SOC Deficiency Index (0.0: High Carbon → 1.0: Severe Deficiency)', color='white', fontsize=11, labelpad=10)
    ...
    high_risk_pct = (np.sum(valid_risk > HIGH_CUTOFF) / len(valid_risk)) * 100.0 if len(valid_risk) > 0 else 0

    ax.text(0.02, 0.02, f"Satellite RF SOC Model | Mean SOC Def: {mean_risk:.3f} | High Def Area (>{HIGH_CUTOFF}): {high_risk_pct:.1f}%",
    ...
    print(f"[OK] Saved SOC Deficiency Index Map to: {out_path}")
    return out_path
...
def plot_risk_histogram(valid_risk, output_dir=OUTPUT_DIR):
    ...
    for i in range(len(patches)):
        ...
        if b_center < LOW_CUTOFF:
            patches[i].set_facecolor('#22c55e')  # Green (Low Risk)
        elif b_center < HIGH_CUTOFF:
            patches[i].set_facecolor('#eab308')  # Yellow (Moderate Risk)
        else:
            patches[i].set_facecolor('#ef4444')  # Red (High Risk)

    ax.axvline(LOW_CUTOFF, color='#eab308', linestyle='--', linewidth=1.5, label=f'Moderate Cutoff ({LOW_CUTOFF})')
    ax.axvline(HIGH_CUTOFF, color='#ef4444', linestyle='--', linewidth=1.5, label=f'High Risk Cutoff ({HIGH_CUTOFF})')

    ax.set_title("Raipur AOI – Re-calibrated Soil Health Risk Score Distribution", fontsize=13, fontweight='bold', color='white', pad=10)
    ...
    print(f"[OK] Saved Risk Histogram to: {out_path}")
    return out_path
...
def run_phase3():
    ...
    table_imp = Table(title="Random Forest Feature Importances", border_style="cyan")
    ...
    low_risk_mask = valid_risk < LOW_CUTOFF
    mod_risk_mask = (valid_risk >= LOW_CUTOFF) & (valid_risk <= HIGH_CUTOFF)
    high_risk_mask = valid_risk > HIGH_CUTOFF

    low_risk_ha = np.sum(low_risk_mask) * pixel_area_ha
    mod_risk_ha = np.sum(mod_risk_mask) * pixel_area_ha
    high_risk_ha = np.sum(high_risk_mask) * pixel_area_ha
    ...
    stats_data = [
        ...
        {'Metric': f'Low Risk Area (<{LOW_CUTOFF}, ha)', 'Value': round(low_risk_ha, 2)},
        {'Metric': 'Low Risk Area (%)', 'Value': round((low_risk_ha / bare_soil_ha) * 100, 2)},
        {'Metric': f'Moderate Risk Area ({LOW_CUTOFF}-{HIGH_CUTOFF}, ha)', 'Value': round(mod_risk_ha, 2)},
        {'Metric': 'Moderate Risk Area (%)', 'Value': round((mod_risk_ha / bare_soil_ha) * 100, 2)},
        {'Metric': f'High Risk Area (>{HIGH_CUTOFF}, ha)', 'Value': round(high_risk_ha, 2)},
    ...
    table_summary.add_row(f"Low Risk (< {LOW_CUTOFF})", f"{low_risk_ha:,.2f} ha", f"{(low_risk_ha/bare_soil_ha)*100:.2f}%")
    table_summary.add_row(f"Moderate Risk ({LOW_CUTOFF} - {HIGH_CUTOFF})", f"{mod_risk_ha:,.2f} ha", f"{(mod_risk_ha/bare_soil_ha)*100:.2f}%")
    table_summary.add_row(f"High Risk (> {HIGH_CUTOFF})", f"{high_risk_ha:,.2f} ha", f"{(high_risk_ha/bare_soil_ha)*100:.2f}%")
